...
var Highlight = function Highlight(wrap) {
    this.wrap = wrap;
    this.bind();
};
...
module.exports = {
    calculateSlotSizes: calculateSlotSizes
};
...
var $ = __webpack_require__(0);
var Highlight = __webpack_require__(1);
var timegrid = __webpack_require__(2);
...
$(document).ready(function () {
    var editForm = document.querySelector('form.edit-schedule');
    if (editForm) {
        var _previewContainer = editForm.querySelector('.preview');
        var formData = parseForm(formValues(editForm));
        _previewContainer.innerHTML = makePreview(formData);
        ...
        var repeatToggle = editForm.querySelector('[name="does_slot_repeat"]');
        var slotRepeat = editForm.querySelector('[name="slot_repeat"]');
        var slotDuration = parseTime(formData.duration);
        var totalLength = parseTime(formData.end_time) - parseTime(formData.start_time);
...
function toggleRepeatInput(e) {
    var editForm = document.querySelector('form.edit-schedule');
    var slotRepeatContainer = editForm.querySelector('.slot-repeat');
    var slotRepeatField = editForm.querySelector('[name="slot_repeat"]');
...
function updateEndTime() {
    var editForm = document.querySelector('form.edit-schedule');
    var slotRepeats = parseInt(editForm.querySelector('[name="slot_repeat"]').value || 1);
    var formData = parseForm(formValues(editForm));
    var duration = parseTime(formData.duration);
    var shiftLength = duration * slotRepeats;
    var endTime = parseTime(formData.start_time) + shiftLength;
...
function updatePreview() {
    var editForm = document.querySelector('form.edit-schedule');
    var formData = parseForm(formValues(editForm));
    var previewContainer = editForm.querySelector('.preview');
...
function formValues(form) {
    ...
    var inputTags = [].concat(_toConsumableArray(form.querySelectorAll('input'))).reduce(function getInputValues(field, e) {
        var name = e.name,
            value = e.value,
            checked = e.checked;
    ...
    var selectTags = [].concat(_toConsumableArray(form.querySelectorAll('select'))).reduce(function getSelectValues(field, e) {
        if (e.name) {
            //const selection = e.querySelector('option[selected]');
            var name = e.name,
                value = e.value;

            var selectedOption = e.querySelector('option[value="' + value + '"]');
    ...
    var fields = Object.assign({}, inputTags, selectTags);
...
function parseForm(formValues) {
    ...
    if (!formValues.department_id || !formValues.shift_id || !formValues.start_time || !formValues.end_time || !formValues.duration || !formValues.volunteers || !formValues['dates[]']) {
        return null;
    }
    var days = formValues['dates[]'].values.filter(function getChecked(date) {
        return date.checked;
    ...
    var volunteers = formValues.volunteers.value || null;

    // these could be abstracted to be more concise
    // since the logic is duplicated
    var start_time = formValues.start_time.value || null;
    var end_time = formValues.end_time.value || null;
    var duration = formValues.duration.value || null;
    ...
    var formData = (_formData = {
        ...
        duration: duration,
        end_time: end_time
    }, _defineProperty(_formData, 'duration', duration), _defineProperty(_formData, 'volunteers', volunteers), _formData);
...
function normalizeTime(time) {
    ...
    var segments = time.split(':').map(function formatSegment(segment) {
...
function makePreview(data) {
    ...
    var start = parseTime(normalizeTime(data.start_time));
    var slotDuration = parseTime(normalizeTime(data.duration));
    var end = parseTime(normalizeTime(data.end_time));
    var volunteers = parseInt(data.volunteers);

    var slots = [];
    for (var currentTime = start; currentTime < end; currentTime += slotDuration) {
        if (currentTime + slotDuration <= end) //don't create shifts which go beyond the end date
            {
                for (var row = 1; row <= volunteers; row++) {
                    slots = slots.concat({
                        id: null,
                        row: row,
    ...
    var days = data.days.reduce(function createDays(days, day) {
        return days.concat({
            name: day.name,
            date: day.date,
            departments: [{
...
$(document).ready(function () {
    ...
    $('.edit-schedule input[name="does-slot-repeat"]').on('change', function () {
        var slotRepeat = document.querySelector('.slot-repeat');
        if (this.checked) {
...
__p += '\n     <div class="day">\n       <div class="heading">\n         <h3>' +
((__t = ( days[day].name )) == null ? '' : __t) +
'</h3> — <i>' +
((__t = ( days[day].date )) == null ? '' : __t) +
...
((__t = ( days[day].departments[department].id )) == null ? '' : __t) +
'/edit">' +
((__t = ( days[day].departments[department].name )) == null ? '' : __t) +
'</a><br>\n                </div>\n                <ul class="shifts">\n                  ';
...
((__t = ( days[day].departments[department].shifts[shift].id )) == null ? '' : __t) +
'/edit">' +
((__t = ( days[day].departments[department].shifts[shift].name )) == null ? '' : __t) +
'</a>\n                       </div>\n\n                       <div class="slots col-sm-10">\n                         ';
 for(var slot=0; slot < days[day].departments[department].shifts[shift].slots.length; slot++) {;
__p += '\n\n                            <span class="slot-wrap" data-start="' +
((__t = ( days[day].departments[department].shifts[shift].slots[slot].start_date )) == null ? '' : __t) +
'" data-duration="' +
((__t = ( days[day].departments[department].shifts[shift].slots[slot].duration )) == null ? '' : __t) +
'" data-row="' +
((__t = ( days[day].departments[department].shifts[shift].slots[slot].row )) == null ? '' : __t) +
'">\n                           <a class="slot empty" data-id="65" title="' +
((__t = ( days[day].departments[department].shifts[shift].slots[slot].title )) == null ? '' : __t) +
'">' +
((__t = ( days[day].departments[department].shifts[shift].slots[slot].title )) == null ? '' : __t) +
'</a>\n                         </span>\n                         ';
 } ;
